...
namespace Scrips
{
     public class PathFinder
     {
        ...
        public List<Vector3> FindPath(Vector3 s, Vector3 g, TraversabilityManager traversabilityManager)
        {
            ...
            var start = new Node(new DiscreteConfig(traversability.get_i_index(s.x), traversability.get_j_index(s.z))); 
            var goal = new Node(new DiscreteConfig(traversability.get_i_index(g.x), traversability.get_j_index(g.z)));

            start = placeNodeOutsideOfCSO(start);
            goal = placeNodeOutsideOfCSO(goal);
            ...
            while (openSet.Count > 0)
            {
                ...
                foreach (var neighbor in Neighbors(current))
                {
                    ...
                    var projectedF = projectedG + Heuristic(neighbor, goal);
                    ...
                    fScore[neighbor] = projectedF;

                }
            }
            
            Debug.LogFormat("OpenSet count {0}", openSet.Count);
        ...
        private double getFScore(Node pt)
        {
         ...
         public bool IsValidNode(Node pt)
         {
        ...
        private Node nextBest()
        {
         ...
         private Node placeNodeOutsideOfCSO(Node node)
         {
             if (!IsValidNode(node))
             {
